...
export class ProductsController {
    ...
    @Get()
    @HttpCode(200)
    getProductsController(@Query('page') page: string, @Query('limit') limit: string) {
        if (page && limit) {
    ...
    @Roles(Rol.Admin)
    @UseGuards(AuthGuard, RolesGuard)
    updateProductController(@Param('id', ParseUUIDPipe) id: string, @Body() data: UpdateProductDto) {
