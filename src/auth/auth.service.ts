...
export class AuthService {
    ...
    async loginUser(credentials: LoginUserDto) {
        const findUser = await this.usersRepository.findOneBy({ email: credentials.email });
        if (!findUser) throw new BadRequestException('Bad Credentials');
        
        const matchingPasswords = await bcrypt.compare(credentials.password, findUser.password);
